...
var visitorEnter = function (abi, state) { return ({
    ObjectTypeDefinition: function (node) {
        ...
        var module = abi_1.createModuleDefinition({
            imports: imports,
            interfaces: (_a = node.interfaces) === null || _a === void 0 ? void 0 : _a.map(function (x) {
                return abi_1.createInterfaceImplementedDefinition({ type: x.name.value });
            }),
    ...
    FieldDefinition: function (node) {
        ...
        var returnType = abi_1.createPropertyDefinition({
            type: type ? type : "N/A",
        ...
        var method = abi_1.createMethodDefinition({
            name: node.name.value,
            return: returnType,
...
var parseCapabilitiesDirective = function (nodeName, node) {
    ...
    try {
        for (var _b = __values(node.directives), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            if (!abi_1.capabilityTypes.includes(typeArg.value.value)) {
                throw Error("@capability directive's \"type\" argument must be one from " + JSON.stringify(abi_1.capabilityTypes) + ". See type " + nodeName + ".");
            }
            ...
            if (!interfacesByNamespace[namespace]) {
                interfacesByNamespace[namespace] = abi_1.createInterfaceDefinition({
                    type: namespace,
                    uri: uri,
                    namespace: namespace,
                    capabilities: abi_1.createCapability({
                        type: capabilityType,
                        enabled: true,
                    }),
                });
...
exports.getModuleTypesVisitor = function (abi) {
    var state = {};
    return {

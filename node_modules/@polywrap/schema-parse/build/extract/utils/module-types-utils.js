...
function extractListType(state) {
    ...
    if (method && argument) {
        // Argument value
        argument.array = abi_1.createArrayDefinition({
            name: argument.name,
            type: "N/A",
            required: state.nonNullType,
        });
        ...
        // Return value
        if (!method.return) {
            method.return = abi_1.createPropertyDefinition({
                type: "N/A",
                name: method.name,
            });
            state.currentReturn = method.return;
        ...
        state.currentReturn.array = abi_1.createArrayDefinition({
            type: "N/A",
            name: method.name,
            required: state.nonNullType,
        });
...
function extractInputValueDefinition(node, state) {
    ...
    var argument = abi_1.createPropertyDefinition({
        type: type ? type : "N/A",
        name: name,

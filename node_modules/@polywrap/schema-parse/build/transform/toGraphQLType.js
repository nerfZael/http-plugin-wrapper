...
function toGraphQL(def, prefixed) {
    ...
    switch (def.kind) {
        case abi_1.DefinitionKind.Object:
        case abi_1.DefinitionKind.ObjectRef:
        case abi_1.DefinitionKind.Scalar:
        case abi_1.DefinitionKind.ImportedObject:
            return applyRequired(def.type, def.required);
        case abi_1.DefinitionKind.Enum:
        case abi_1.DefinitionKind.EnumRef:
        case abi_1.DefinitionKind.ImportedEnum:
            if (prefixed) {
                return applyRequired("Enum_" + def.type, def.required);
            }
            return applyRequired(def.type, def.required);
        case abi_1.DefinitionKind.Any:
        case abi_1.DefinitionKind.Property:
            return anyToGraphQL(def, prefixed);
        case abi_1.DefinitionKind.Array: {
            var array = def;
            if (!array.item) {
        ...
        case abi_1.DefinitionKind.Map: {
            ...
            return applyRequired("Map<" + toGraphQL(map.key, prefixed) + ", " + anyToGraphQL(map, prefixed) + ">", map.required);
        }
        case abi_1.DefinitionKind.Method: {
            var method = def;
            if (!method.return) {
        ...
        case abi_1.DefinitionKind.Module:
            return def.type;
        case abi_1.DefinitionKind.ImportedModule:
            return def.type;
        default:

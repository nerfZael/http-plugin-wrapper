        ...
        visitor: {
            enter: {
                ...
                ScalarTypeDefinition: function (node) {
                    if (node.name.value !== "Map" && !abi_1.isScalarType(node.name.value)) {
                        throw Error("Custom scalar types are not supported. Found: \"" + node.name.value + "\". Supported scalars: " + abi_1.scalarTypeNames);
                    }
                },
                // No Unions
            ...
            enter: {
                ...
                InputValueDefinition: function (node) {
                    var typeName = currentImportType
                        ? currentImportType
                        : currentObject;
                    if (typeName && !abi_1.isModuleType(typeName)) {
        ...
        cleanup: function () {
            ...
            try {
                ...
                for (var fieldTypes_1 = __values(fieldTypes), fieldTypes_1_1 = fieldTypes_1.next(); !fieldTypes_1_1.done; fieldTypes_1_1 = fieldTypes_1.next()) {
                    ...
                    if (!abi_1.isScalarType(field.type) &&
